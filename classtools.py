...
def get_extent(data):
    try:
        if isinstance(data, arcpy.Result):
            data = data.getOutput(0)
        if isinstance(data, arcpy.Extent):
            ...
            return data.extent
        elif isinstance(data, arcpy._mp.Layer):
            limits = Limits()
            for row in arcpy.da.SearchCursor(data, 'SHAPE@'):
                limits.union(get_extent(row[0]))
            return limits.to_extent()
        else:
...
class Limits(object):
    ...
    def to_extent(self):
        return arcpy.Extent(self.data[0], self.data[2], self.data[1], self.data[3])
...
class Plotter(object):
    ...
    def plot(self, data, symbols, margin, extent=None, **kwargs):
        ...
        for d, s in zip(data, symbols):
            try:
                if isinstance(d, arcpy.Result):
                    d = d.getOutput(0)
                if isinstance(d, str) or isinstance(d, arcpy._mp.Layer):
